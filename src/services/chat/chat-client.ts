...
import type { ChatMessage, ChatResponse, ChatSendResult } from "./types";
...
export async function sendChat(options: SendChatOptions): Promise<ChatSendResult> {
  ...
  while (true) {
    ...
    const assistant = assistantFromAiBrainReply(payload);
    const conversationId = payload && typeof payload === "object" && "conversationId" in payload && typeof (payload as { conversationId?: unknown }).conversationId === "string" ? (payload as { conversationId: string }).conversationId : options.conversationId || "";
