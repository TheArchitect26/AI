...
export const Route = createFileRoute("/api")({
  head: () => ({
    meta: [
      { title: "API Manager — NEXUS" },
      { name: "description", content: "Keys, rate limits, webhooks" },
    ],
  }),
  component: Page,
});
      ...
      <GlassPanel className="relative overflow-hidden p-6" scanline>
        ...
        <div className="relative">
          <div className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.22em] text-neon-cyan">
            <StatusDot pulse /> Integrations
          </div>
          <h1 className="mt-2 text-2xl font-semibold tracking-tight md:text-3xl">API Manager</h1>
          <p className="mt-1 max-w-2xl text-sm text-muted-foreground">Keys, rate limits, webhooks</p>
        </div>
      </GlassPanel>
