...
export const Route = createFileRoute("/tasks")({
  head: () => ({
    meta: [
      { title: "Task Orchestrator — NEXUS" },
      { name: "description", content: "Active, queued, dependency graphs" },
    ],
  }),
  component: Page,
});
      ...
      <GlassPanel className="relative overflow-hidden p-6" scanline>
        ...
        <div className="relative">
          <div className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.22em] text-neon-cyan">
            <StatusDot pulse /> Planning chains
          </div>
          <h1 className="mt-2 text-2xl font-semibold tracking-tight md:text-3xl">Task Orchestrator</h1>
          <p className="mt-1 max-w-2xl text-sm text-muted-foreground">Active, queued, dependency graphs</p>
        </div>
      </GlassPanel>
