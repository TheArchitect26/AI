...
export const Route = createFileRoute("/infra")({
  head: () => ({
    meta: [
      { title: "Infrastructure — NEXUS" },
      { name: "description", content: "CPU, GPU, VRAM, containers, latency" },
    ],
  }),
  component: Page,
});
      ...
      <GlassPanel className="relative overflow-hidden p-6" scanline>
        ...
        <div className="relative">
          <div className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.22em] text-neon-cyan">
            <StatusDot pulse /> Datacenter
          </div>
          <h1 className="mt-2 text-2xl font-semibold tracking-tight md:text-3xl">Infrastructure</h1>
          <p className="mt-1 max-w-2xl text-sm text-muted-foreground">CPU, GPU, VRAM, containers, latency</p>
        </div>
      </GlassPanel>
