...
export const Route = createFileRoute("/workspace")({
  head: () => ({
    meta: [
      { title: "Code Workspace — NEXUS" },
      { name: "description", content: "Monaco-powered AI pair-programming workspace" },
    ],
  }),
  component: Page,
});
...
function Page() {
  ...
  const [chat, setChat] = useState<{ role: "user" | "ai"; text: string }[]>([
    {
      role: "ai",
      text: "Workspace ready. Open a file, edit it, or describe a change and I'll rewrite the active file with AI.",
    },
  ]);
  ...
  const onMount = useCallback((ed: editor.IStandaloneCodeEditor, m: Monaco) => {
    ...
    // doSave reads current refs and active path at command execution time.
  ...
  const onChange = (v?: string) => {
    setFiles((fs) =>
      fs.map((f) =>
        f.path === activePath ? { ...f, content: v ?? "", dirty: f.content !== (v ?? "") } : f,
      ),
    );
  };
...
${active.content}`;
      ...
      const cleaned = response.assistant.content
        .replace(/^```[\w-]*\n?/, "")
        .replace(/\n?```\s*$/, "")
        .trim();
      if (cleaned) {
        setFiles((fs) =>
          fs.map((f) => (f.path === activePath ? { ...f, content: cleaned, dirty: true } : f)),
        );
        editorRef.current?.setValue(cleaned);
        setChat((c) => [
          ...c,
          { role: "ai", text: `Applied edit to \`${active.path}\`. Review and ⌘S to save.` },
        ]);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
        <GlassPanel className="lg:col-span-2">
          <SectionHeader
            ...
            right={
              <button
                onClick={newFile}
                className="rounded p-1 text-muted-foreground hover:text-neon-cyan"
              >
          ...
          <div className="space-y-0.5 p-2 text-xs">
            {files.map((f) => (
              <button
                ...
                className={cn(
                  "flex w-full items-center gap-1.5 rounded px-1.5 py-1 text-left font-mono",
                  f.path === activePath
                    ? "bg-[color-mix(in_oklab,var(--neon-cyan)_12%,transparent)] text-foreground"
              ...
              >
                ...
                {f.dirty && (
                  <span className="ml-auto h-1.5 w-1.5 shrink-0 rounded-full bg-[color:var(--neon-amber)]" />
                )}
              </button>
            ))}
          </div>
        ...
        <GlassPanel className="flex min-h-[34rem] flex-col lg:col-span-7">
          <SectionHeader
            eyebrow="Editor"
            title={active.path + (active.dirty ? " •" : "")}
            right={
              <div className="flex items-center gap-2">
                <GitBranch className="h-3 w-3 text-muted-foreground" />
                <span className="font-mono text-[10px] text-muted-foreground">main</span>
                <NeonButton variant="ghost" onClick={triggerSearch}>
                  <Search className="h-3 w-3" /> Find
                </NeonButton>
                <NeonButton variant="ghost" onClick={doSave}>
                  <Save className="h-3 w-3" /> Save
                </NeonButton>
                <NeonButton
                  onClick={() => setTerminal((t) => [...t, `$ run ${active.path}`, "▶ ok"])}
                >
          ...
          <div className="border-t border-border/60">
            <div className="flex items-center gap-2 px-4 py-1.5 font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground">
            ...
            <div className="max-h-32 space-y-0.5 overflow-y-auto px-4 pb-3 font-mono text-[11px]">
              {terminal.map((l, i) => (
                <div
                  key={i}
                  className={cn(
                    l.startsWith("$") && "text-neon-cyan",
                    l.startsWith("✓") && "text-[color:var(--neon-lime)]",
                    (l.startsWith("→") || l.startsWith("▶")) && "text-neon-violet",
                  )}
                >
                  {l}
                </div>
          ...
          <SectionHeader
            ...
            right={
              <div className="flex items-center gap-1.5">
                <StatusDot tone={aiBusy ? "online" : "idle"} pulse={aiBusy} />
                ...
                <div
                  ...
                  className={cn(
                    ...
                    m.role === "user"
                      ...
                      : "border-neon-cyan/30 bg-[color-mix(in_oklab,var(--neon-cyan)_6%,transparent)]",
                  )}
                >
